...
# 		if libraries[i]['output_number_of_books'] < 1:
# 			libraries[i]['checking'] = False
# 	for i in range(len(signed_up)):
# 		if libraries[signed_up[i]]['checking'] == True:
# 			signed_up_valid.append(signed_up[i])
...
# 	signed_up_valid = [] # A list containing the IDs of the libraries which are valid and signed up
...
# 	for i in range(len(signed_up_valid)):
# 		if libraries[signed_up_valid[i]]['checking']:
# 			if i != len(signed_up_valid) - 1:
# 				output.write(str(signed_up_valid[i]) + " " + str(libraries[signed_up_valid[i]]['output_number_of_books']) + '\n')
# 				for j in range(len(libraries[signed_up_valid[i]]['scanned_books'])):
# 					if j == len(libraries[signed_up_valid[i]]['scanned_books']) - 1:
# 						output.write(str(libraries[signed_up_valid[i]]['scanned_books'][j]) + '\n')
# 					else:
# 						output.write(str(libraries[signed_up_valid[i]]['scanned_books'][j]) + ' ')
# 			else:
# 				output.write(str(signed_up_valid[i]) + " " + str(libraries[signed_up_valid[i]]['output_number_of_books']) + '\n')
...
# --------------------------------------------------REWRITE OF THE SAME CODE BELOW -----------------------------------------------------------
...
def main(inputfile, outputfile):
	with open(inputfile) as f:
		...
		for i in range(number_of_libraries):
			...
			sorted_library_books = sorting(library_books, book_scores)
			libraries[i] = {
			...
			"checking": True,
			"ratio": (sum(list(map(lambda x: book_scores[x], library_books))) / details[1])
			}

	signup(libraries, outputfile)

# Data Writing Method

def output_writing(outputfile, signed_up_valid, libraries):
...
def signup(libraries, outputfile):
	days_taken = 0
	library_score_temp = []
	for i in libraries:
	...
	bookscan(libraries, outputfile)

# Book Scanning Method

def bookscan(libraries, outputfile):
	books_to_scan = 0
	for i in signed_up:
		i_books = libraries[i]['books']
		if libraries[i]['starting_day'] >= number_of_days:
	...
	output_writing(outputfile, signed_up_valid, libraries)

number_of_books, number_of_libraries, number_of_days = 0, 0, 0
book_scores = {}
signed_up = []
signed_up_valid = []
books = []
...
main("a_example.txt", "a_output.txt")
main("b_read_on.txt", "b_output.txt")
main("c_incunabula.txt", "c_output.txt")
main("d_tough_choices.txt", "d_output.txt")
main("e_so_many_books.txt", "e_output.txt")
main("f_libraries_of_the_world.txt", "f_output.txt")